...
namespace MailMeUpTestMeUp.LogsTest
{
    ...
    public class DeleteAllLogsOfSession : SuperUnitTest
    {
        [Theory]
        [InlineData("Boss", "Boss", true, true)]
        [InlineData("", "", false, true)]
        [InlineData("BossWrong", "BossWrong", false, true)]
        public async Task DeleteAllLogsOfSessionTest(string username, string password, bool correctCreds, bool succesfulRequest)
        {
            ...
            if (!string.IsNullOrEmpty(token))
            {
                ...
                if (response.Success)
                {
                    ...
                    if (session != null)
                    {
                        ...
                        var result = await _lib.Log.DeleteAllLogsOfSession(sessionGuid, token);
